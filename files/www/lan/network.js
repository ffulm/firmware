...
function appendSetting(p, path, value, mode)
{
	...
	case "txpower":
		...
		addHelpText(b, "The transmission power in dBm. Radiation power = transmission power - cable attenuation + antenna gain. \ NOther values must be entered manually. Attention! When replacing the antennas, the transmission power must be adjusted accordingly!");
		addClass(b, "adv_hide");
		break;
	case "mode":
	...
	case "mesh_id":
		...
		if (!inArray(mode, ["wan", "lan", "none"]))
			addClass(b.lastChild, "adv_disable");
		addInputCheck(b.lastChild, /^[^\x00-\x1F\x80-\x9F]{3,30}$/, "Mesh ID is invalid.");
		break;
	case "ssid":
		b = append_input(p, "SSID", id, value);
		if (!inArray(mode, ["wan", "lan", "none"]))
			addClass(b.lastChild, "adv_disable");
		addInputCheck(b.lastChild, /^[^\x00-\x1F\x80-\x9F]{3,30}$/, "SSID is invalid.");
		break;
/*
	case "macaddr":
...
*/
	case "mesh_on_wan":
		...
		break;
	case "disabled":
		b = append_radio(p, "Disabled", id, value, [["Ja", "1"], ["Nein", "0"]]);
		break;
	default:
		return;
	}
...
function rebuild_switches()
{
	...
	config_foreach(uci.network, "switch", function(sid, sobj) {
		...
		if (!swinfo.map) {
			...
			label.innerHTML = "No port configuration possible.";
